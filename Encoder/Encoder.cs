...
namespace SpatialClusteringEncoder
{
	...
	sealed class LayersProcessorJob
	{
		...
		public string resultIndirectDescription;
		public string resultIndirectMap;
		public string resultWeightsMap;
		...
		public void PrintParameters()
		{
			...
			Debug.Log(string.Format("    {0}", subsetsId));
			...
			Debug.Log(string.Format("    {0}", resultIndirectDescription));
			Debug.Log(string.Format("    {0}", resultIndirectMap));
			Debug.Log(string.Format("    {0}", resultWeightsMap));
			Debug.Log("");
		}
	}
	...
	sealed class LayersProcessor
	{
		...
		bool LoadColorIdMap()
		{
			...
			Debug.Log(string.Format("  Load subsets id_map '{0}'", job.subsetsId));

			matBaseLayer = CpuTexture2D.LoadFromFile(job.subsetsId);
 ...
 */ 
		...
		bool SaveResults()
		{
			TgaFormat.Save(job.resultIndirectMap, indirectionMap.texels, false, indirectionMap.width, indirectionMap.height);

			bool needAlpha = (job.maxLocalLayersCount == 5);
			TgaFormat.Save(job.resultWeightsMap, weightsMap.texels, needAlpha, weightsMap.width, weightsMap.height);

			using (StreamWriter writer = File.CreateText(job.resultIndirectDescription))
			{
				writer.WriteLine("<?xml version=\"1.0\"?>");
				writer.WriteLine("<Palettes>");
				for (int j = 0; j < localPalettes.Count; j++)
				{
					writer.WriteLine("  <Palette id=\"{0}\">", (j + 1));
					...
					foreach (int layerID in palette.sourceIds)
					{
						if (layerID == 63)
						{
							...
							string baseLayerName = string.Format("base_{0:X2}{1:X2}{2:X2}", palette.baseLayerColor.r, palette.baseLayerColor.g, palette.baseLayerColor.b);

							writer.WriteLine("    <BaseLayer name=\"{0}\" />", baseLayerName);
						}
						else
						{
							writer.WriteLine("    <Layer name=\"{0}\" />", job.sourceLayers[layerID]);
						}
					}
				...
				writer.WriteLine("// bad palette index");
				writer.WriteLine("//////////////////////////////////////////////////////////////////////");
				...
				{
					writer.WriteLine("");
					writer.WriteLine("palettes[0].m{0}.color = float4(1, 0, 0, 1);", index);
					writer.WriteLine("palettes[0].m{0}.glossiness = 1;", index);
					writer.WriteLine("palettes[0].m{0}.metallic = 1;", index);
					writer.WriteLine("palettes[0].i{0} = 0;", index);
				}


				for (int paletteIndex = 0; paletteIndex < 15; paletteIndex++)
				{
					writer.WriteLine("");
					writer.WriteLine("// palette {0}", paletteIndex);
					writer.WriteLine("//////////////////////////////////////////////////////////////////////");

					LocalPalette palette = null;

					if (paletteIndex < localPalettes.Count)
					{
						palette = localPalettes[paletteIndex];
					}

					index = 0;
					if (palette != null)
					{
						for (index = 0; index < palette.sourceIds.Count; index++)
						{
							int srcPaletteIndex = palette.sourceIds[index];

							int srcIndex = (srcPaletteIndex + 1);
							if (srcIndex == 64)
							{
								srcIndex = 0;
							}
							...
							if (srcPaletteIndex != 63)
							{
								writer.WriteLine("// source layer '{0}'", job.sourceLayers[srcPaletteIndex]);
							} else
							{
								writer.WriteLine("// source layer = base layer'");
							}
							writer.WriteLine("palettes[{0}].m{1}.color = _Color{2};", (paletteIndex+1), index, srcIndex);
							writer.WriteLine("palettes[{0}].m{1}.glossiness = _Glossiness{2};", (paletteIndex + 1), index, srcIndex);
							writer.WriteLine("palettes[{0}].m{1}.metallic = _Metallic{2};", (paletteIndex + 1), index, srcIndex);
							writer.WriteLine("palettes[{0}].i{1} = {2};", (paletteIndex + 1), index, srcIndex);
						}
					}
					...
					{
						writer.WriteLine("");
						writer.WriteLine("palettes[{0}].m{1}.color = 0;", (paletteIndex + 1), index);
						writer.WriteLine("palettes[{0}].m{1}.glossiness = 0;", (paletteIndex + 1), index);
						writer.WriteLine("palettes[{0}].m{1}.metallic = 0;", (paletteIndex + 1), index);
						writer.WriteLine("palettes[{0}].i{1} = 0;", (paletteIndex + 1), index);
					}
				}
